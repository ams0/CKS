...
#Deploy the cluster
sudo kubeadm init  --control-plane-endpoint ${api_endpoint} --token ${token} --cri-socket /run/containerd/containerd.sock --pod-network-cidr=10.244.0.0/16

#Get kubeconfig
mkdir -p $HOME/.kube
sudo cp -i /etc/kubernetes/admin.conf $HOME/.kube/config
sudo chown $(id -u):$(id -g) $HOME/.kube/config
sudo chmod a+rw $HOME/.kube/config
